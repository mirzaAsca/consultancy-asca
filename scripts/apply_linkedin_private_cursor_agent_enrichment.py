#!/usr/bin/env python3
"""Merge web-researched /in/ URLs from agent-owned JSON into linkedin-private-cursor.

Reads only:
  data-cache/linkedin_private_cursor_agent_enrichment.json

Does not read or modify linkedin_private_cursor_manual_mappings.json.

Behavior: for each dataset row, union existing linkedin-private-cursor URLs with URLs
matched from this file's firms / people / companies (same key rules as the repo's
manual apply script). Writes CSVs only when the merged cell changes.
"""
...
AGENT_MAPPINGS_PATH = REPO_ROOT / "data-cache" / "linkedin_private_cursor_agent_enrichment.json"
...
def load_agent_mappings() -> tuple[dict[str, list[str]], dict[str, list[str]], dict[str, list[str]]]:
    if not AGENT_MAPPINGS_PATH.exists():
        return {}, {}, {}
    with AGENT_MAPPINGS_PATH.open(encoding="utf-8") as handle:
...
def merge_cursor(existing_cell: str, agent_urls: list[str]) -> str:
    merged = dedupe_urls(parse_cursor_cell(existing_cell) + agent_urls)
    return ", ".join(merged)
...
def main() -> int:
    if not DATASETS_DIR.is_dir():
        ...
        return 1
    firm_map, people_map, company_map = load_agent_mappings()
    if not firm_map and not people_map and not company_map:
        print(f"No mappings in {AGENT_MAPPINGS_PATH}", file=sys.stderr)
        return 0
    ...
    for csv_path in sorted(DATASETS_DIR.glob("*.csv")):
        ...
        changed = 0
        for row in rows:
            agent_urls = agent_target_urls(row, firm_map, people_map, company_map)
            if not agent_urls:
                continue
            prev = (row.get("linkedin-private-cursor") or "").strip()
            merged = merge_cursor(prev, agent_urls)
            if merged == prev:
                continue
