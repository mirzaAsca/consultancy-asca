#!/usr/bin/env python3
"""Apply web-researched personal LinkedIn URLs into linkedin-private-cursor across datasets/*.csv.

Mappings live in data-cache/linkedin_private_cursor_manual_mappings.json:
  - "firms": fund display name -> list of /in/ URLs (whole team; applied when firm name matches a row).
  - "people": person lookup keys -> /in/ URLs (narrower matches; see person_keys).
  - "companies": LinkedIn company page keys -> /in/ URLs. Keys are normalized paths like
    "linkedin.com/company/first-round-capital". Any cell in a row that contains that company URL
    contributes those personal URLs (covers rows that only store /company/ links).

All matching sources for a row are merged and de-duplicated (people + company URLs in cells + firm name).

Run after updating the JSON (via manual web research or agent batches). Idempotent for unchanged values.
"""
import csv
import json
import re
...
MAPPINGS_PATH = Path("data-cache/linkedin_private_cursor_manual_mappings.json")
...
def sanitize_company_slug(fragment: str) -> str:
    """Strip trailing junk (e.g. comma + flag emoji) from company URL slugs in scraped CSVs."""
...
def load_mappings() -> tuple[dict[str, list[str]], dict[str, list[str]], dict[str, list[str]]]:
...
def main() -> None:
    firm_map, people_map, company_map = load_mappings()
    changed_files: dict[str, int] = {}

    for csv_path in sorted(DATASETS_DIR.glob("*.csv")):
        # utf-8-sig so UTF-8 BOM does not produce a bogus "\ufeffName" field (breaks Name/Fund matching).
        ...
        changed = 0
        for row in rows:
            urls = target_urls(row, firm_map, people_map, company_map)
            if not urls:
                continue
            value = ", ".join(urls)
            if (row.get("linkedin-private-cursor") or "").strip() == value:
                continue
            row["linkedin-private-cursor"] = value
            changed += 1
