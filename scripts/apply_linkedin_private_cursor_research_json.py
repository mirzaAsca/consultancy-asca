#!/usr/bin/env python3
"""Merge web-researched /in/ URLs from a session JSON in research/ into linkedin-private-cursor.

Reads a single JSON path (default: research/linkedin_private_cursor_session_2026_04_18.json).
Schema matches data-cache agent enrichment: meta, firms, people, companies (all optional).

Does not read or write other agents' mapping files.

Usage:
  python3 scripts/apply_linkedin_private_cursor_research_json.py
  python3 scripts/apply_linkedin_private_cursor_research_json.py research/my_batch.json
"""
...
DEFAULT_RESEARCH_JSON = REPO_ROOT / "research" / "linkedin_private_cursor_session_2026_04_18.json"
...
def clean_url(url: str) -> str:
    ...
    if "/in/" not in url or "linkedin.com" not in url.lower():
        return ""
    if "/pub/dir" in url:
        return ""
    return url
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply research JSON into linkedin-private-cursor.")
    parser.add_argument(
        "json_path",
        nargs="?",
        type=Path,
        default=DEFAULT_RESEARCH_JSON,
        help=f"Path to research JSON (default: {DEFAULT_RESEARCH_JSON.relative_to(REPO_ROOT)})",
    )
    ...
    firm_map, people_map, company_map = load_mappings(json_path)
    if not firm_map and not people_map and not company_map:
        print(f"No mappings in {json_path}", file=sys.stderr)
        return 0
    ...
    for csv_path in sorted(DATASETS_DIR.glob("*.csv")):
        ...
        changed = 0
        for row in rows:
            found = target_urls(row, firm_map, people_map, company_map)
            if not found:
                continue
    ...
    print(json.dumps({"source": str(json_path.relative_to(REPO_ROOT)), "changed": changed_files}, indent=2))
    return 0
