...
describe('recommendAction', () => {
  it('warms 2nd-degree with no history when warm_visit flag is on', () => {
    ...
    const result = recommendAction(p, [], { warm_visit_before_invite: true });
    expect(result).toBe('profile_visit');
  });

  it('skips warming when disabled → straight to invite', () => {
    const p = makeProspect({ level: '2nd' });
    const result = recommendAction(p, [], { warm_visit_before_invite: false });
    expect(result).toBe('connection_request_sent');
  });

  it('recommends invite after a prior warming visit', () => {
    const p = makeProspect({ level: '2nd' });
    const history = [makeAction({ kind: 'profile_visit', state: 'sent' })];
    const result = recommendAction(p, history, { warm_visit_before_invite: true });
    expect(result).toBe('connection_request_sent');
  });
  ...
  it('suppresses invite when one is already live (pending)', () => {
    ...
    const result = recommendAction(p, history, { warm_visit_before_invite: false });
