...
- **Popup** — upload, start / pause / resume, live progress + ETA, 4-tile stats grid, quick export, manual **Feed Crawl Session** button.
- **Feed Crawl Session** — manual two-mode (Top → Recent) DOM-walk inside a dedicated LinkedIn feed tab opened by the extension. Scrolls with gentle jitter (600–1200 px, 2–5 s waits), yields on keyboard / mouse / wheel / touch interaction, and stops after 3 consecutive empty scrolls or 20 total steps per mode. Logged to `activity_log` (`feed_crawl_session_start` / `_end` with per-mode metrics + overlap count) so the harvest trail shows up in the dashboard Logs tab.
