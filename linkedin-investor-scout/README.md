...
- **Feed Crawl Session** — manual two-mode (Top → Recent) DOM-walk inside the active LinkedIn feed tab. Scrolls with gentle jitter (600–1200 px, 2–5 s waits), yields on any user interaction (keyboard / mouse / wheel / scroll), and stops after 3 consecutive empty scrolls or 20 total steps per mode. Logged to `activity_log` (`feed_crawl_session_start` / `_end` with per-mode metrics + overlap count) so the harvest trail shows up in the dashboard Logs tab.
