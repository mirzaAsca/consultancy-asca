...
- [x] Pre-invite profile visit warming:
...
- [x] **No scheduled hidden background harvest tab** — passive harvesting still requires a user LinkedIn tab. Revisit hidden/background harvest only if coverage proves insufficient after 30+ days of clean operation. _(Manual sessions are the exception by design: they are explicit user-triggered runs in a visible dedicated tab, not a silent scheduler.)_
- [x] Stop conditions:
  ...
  - [x] user started typing / clicking / touching / wheeling in the crawl tab _(keydown / mousedown / touchstart / wheel flip `user_interacted`; `shouldStopCrawl` returns `'user_interaction'`. Raw scroll events are ignored to avoid false positives from LinkedIn reflow and crawler `scrollBy`.)_.
...
- [x] Surface "newly unlocked 2nd-degree" in queue with highest priority. _(Implemented as a flat +`SCORE_WEIGHTS.recent_unlock_boost` bonus on 2nd-degree rows within `recent_unlock_days` of the transition. Promotes borderline rows into a higher tier for the window. **Visible badge landed 2026-04-26** — new pure predicate `isRecentlyUnlocked(level, last_level_change_at, now)` in [`src/shared/scoring.ts`](./src/shared/scoring.ts) (mirror of the score branch so they can't drift); `OutreachQueueCandidate` ([`src/shared/types.ts`](./src/shared/types.ts)) gains `recent_unlock: boolean` stamped during `buildCandidates` ([`src/shared/outreach-queue.ts`](./src/shared/outreach-queue.ts)). Outreach Queue UI ([`src/dashboard/routes/OutreachQueue.tsx`](./src/dashboard/routes/OutreachQueue.tsx)) renders an emerald "✨ New" pill next to the level badge when set, with a tooltip explaining the +25 priority boost. Unit test in [`tests/outreach-queue.test.ts`](./tests/outreach-queue.test.ts) pins the 2nd-degree-only / inside-window-only / null-handling semantics. Closes the gap where the scoring boost was invisible to the user.)_
- [x] Add acceptance watcher:
...
- [x] You can run daily crawl sessions and see net-new events/tasks by mode. _(2026-04-26 hardening: manual sessions now open a visible dedicated feed tab; background-owned per-mode navigation preserves session telemetry across Top → Recent reloads; per-mode metrics and overlap are merged from one-mode content runs. Validated with `bun run test` and `bun run build` under Node 22.19.0; live LinkedIn smoke still belongs in the release checklist.)_
...
- [x] Add lightweight state on task/outreach rows — landed 2026-04-27. `OutreachAction` now carries optional `auto_tracked_at` + `auto_tracked_source` (`OutreachAutoTrackSource = 'send_detector' | 'message_detector' | 'profile_visit_detector'`) in [`src/shared/types.ts`](./src/shared/types.ts), mirroring the existing `FeedEvent.auto_tracked_*` pattern. Detector callsites (`emitOutreachSent` in [`src/content/outreach-prefill.ts`](./src/content/outreach-prefill.ts) for invite-send; the profile-visit dwell + message-sent emit() calls in [`src/content/interaction-detectors.ts`](./src/content/interaction-detectors.ts)) now pass `auto_tracked_source` on the `OUTREACH_ACTION_RECORD` payload. `handleOutreachActionRecord` in [`src/background/index.ts`](./src/background/index.ts) stamps the fields atomically with the `becameSent` write — both on first-insert and on the existing-draft → sent upgrade path — so the existing idempotency-key dedupe still collapses detector + manual-confirm races onto a single row (manual-confirm wins on the field write, since the auto-track stamp only fires when the inbound payload provides a source). Outreach Queue surfaces a sky-blue "Auto · Xm ago" pill alongside the action label via the new `OutreachQueueCandidate.auto_tracked_today` field — populated by `pickAutoTrackedToday()` in [`src/shared/outreach-queue.ts`](./src/shared/outreach-queue.ts) which scans today's local-day-bucket history for the most-recent auto-tracked sent row. 3 new unit tests in [`tests/outreach-queue.test.ts`](./tests/outreach-queue.test.ts) cover the today-bucket happy path, the prior-day exclusion, and the manual-confirm (no source) skip. Replaces the `interaction_events` lookup the UI would otherwise need on every render.
...
- [x] Persist the correlation token in IDB (`correlation_tokens` store) — survives MV3 service-worker recycles + cross-tab operation (click in dashboard tab, detector fires in LinkedIn tab). Default window 45 min via `CORRELATION_TOKEN_DEFAULT_WINDOW_MS`; Settings-tunable landed 2026-04-26 — new `correlation_token_window_ms` field on `OutreachSettings` ([`src/shared/types.ts`](./src/shared/types.ts)) seeded from `CORRELATION_TOKEN_DEFAULT_WINDOW_MS`, merged forward-compat in [`src/shared/db.ts`](./src/shared/db.ts) `getSettings` so existing v2/v3 installs auto-backfill. `handleInteractionTokenOpen` ([`src/background/index.ts`](./src/background/index.ts)) reads the configured value when the caller doesn't pass an explicit `payload.window_ms` (fallback chain: explicit → settings → module default), with a try/catch around `getSettings()` so a DB-still-booting state can't break the inbox-open path. Settings UI exposes a "Correlation token window (min)" `NumberField` next to the dwell knob in `OutreachCapsSection` ([`src/dashboard/routes/Settings.tsx`](./src/dashboard/routes/Settings.tsx)), bounded `[1, 240]` minutes — the existing 60 s lower clamp in the background handler defends against any sub-1-minute writes.

### 5.3 Action detectors (content script)

- [x] Detect high-confidence manual outcomes:
...
- [~] Auto-transition states — feed_events already flip `new → done` on URN match via the existing Phase 5.3 detector handlers; queue item / outreach row state transitions beyond the existing Mode A + acceptance + withdrawal paths remain open. **Lifecycle-status auto-advance landed 2026-04-26** — pure helper `nextLifecycleAfterOutreachSent` in [`src/shared/lifecycle.ts`](./src/shared/lifecycle.ts) maps every detector- or queue-confirmed `sent` outreach action to the next prospect lifecycle: `profile_visit` advances `new` / `ready_for_visit` → `ready_for_connect`, `connection_request_sent` advances pre-request states (`new` / `ready_for_visit` / `ready_for_connect`) → `request_sent`, `message_sent` / `followup_message_sent` are no-ops (the user is messaging an existing connection — nothing to advance). Forward-only: `do_not_contact` is never overwritten and post-acceptance states (`connected` / `followup_due` / `request_sent`) never regress. Wired in [`src/background/index.ts`](./src/background/index.ts) `handleOutreachActionRecord` after the `becameSent` block, behind a try/catch so a lifecycle write failure can't roll back the outreach record. Logs `lifecycle_status_advanced` to `activity_log` with `{ from, to, kind, action_id }` so the audit trail covers the transition. 21 unit tests in [`tests/lifecycle.test.ts`](./tests/lifecycle.test.ts) pin the no-regress invariants and the do_not_contact lock. Acceptance (request_sent → connected) keeps living in [`src/shared/acceptance-watcher.ts`](./src/shared/acceptance-watcher.ts) since it triggers from a level transition, not from an outreach write. feed_events / outreach_actions row state transitions beyond Mode A still remain open for v2.1. **Invite-expiration sweep landed 2026-04-26** — pure `expireStaleSentInvites(staleDays, now)` in [`src/shared/db.ts`](./src/shared/db.ts) walks the `outreach_actions.by_state` index for `'sent'` rows, flips `connection_request_sent` rows whose `sent_at < now - INVITE_EXPIRATION_DAYS` (default **180**, in [`src/shared/constants.ts`](./src/shared/constants.ts)) to `state='expired'` and stamps `resolved_at`. Wired at the top of `runScanLoop()` in [`src/background/scan-worker.ts`](./src/background/scan-worker.ts) right after the existing `requeueStaleSATierProspects` sweep — same single-flight window, idempotent on re-entry. No budget credit (the slot was burned 6 months ago and is well outside every health/cap window — crediting would silently inflate today's headroom; withdrawals still credit per Phase 5.6). Logs `stale_invites_expired` with `{ count, expiration_days }` when count > 0. 5 unit tests in [`tests/db-v2.test.ts`](./tests/db-v2.test.ts) cover the happy path (cutoff vs in-window), terminal-state + non-invite-kind skips, idempotency on repeated runs, non-positive/NaN window guards, and the `null sent_at` defensive skip. Closes the `sent → expired` FSM edge that was defined in [`src/shared/types.ts`](./src/shared/types.ts) `OutreachActionState` but had no writer — analytics + outreach queue now reflect that 6-month-old invites are terminal, not perpetually pending.
...
- [x] **Message surface coverage.** Message-sent detector now handles all three LinkedIn surfaces:
  ...
  - [x] pop-out / new-window messaging view — landed 2026-04-26. Pop-out windows are top-level browser windows that match the `https://www.linkedin.com/*` host pattern, so the highlight content script injects independently in each (the `all_frames: false` gate only affects subframes within a single document — it does NOT block top-level windows). Two coverage paths now wired: (1) when the pop-out lands at the canonical `/messaging/thread/{id}/` URL, the new `extractMessagingThreadId` pure helper in [`src/shared/url.ts`](./src/shared/url.ts) routes the path to the existing `maybeStartMessageSentDetector` — and now also matches the older `/messaging/messageRoom/{id}/` surface that some accounts still render; (2) when the pop-out renders the slide-out drawer DOM (`aside#msg-overlay`), the existing global `startDrawerMessageWatcher` ([`src/content/interaction-detectors.ts`](./src/content/interaction-detectors.ts) lines 334–358) attaches per-bubble `attachMessageSentWatcher` instances scoped to `.msg-overlay-conversation-bubble`, so multi-bubble pop-outs don't cross-attribute. Both surfaces emit through the same `OUTREACH_ACTION_RECORD { kind: 'message_sent', state: 'sent' }` idempotency path keyed on `{prospect}:message_sent:{day}` so a user toggling between drawer and pop-out can't double-bump `daily_usage.messages_sent`. 8 unit tests in [`tests/url.test.ts`](./tests/url.test.ts) pin the messaging-thread path matcher (thread + messageRoom variants, compose-page miss, nested-path miss, empty-id miss, case-insensitive route).
